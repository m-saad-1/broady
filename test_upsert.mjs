...
async function test() {
  ...
  for (const record of parsed) {
    try {
      ...
      // Attempt upsert with a fake importJobId
      try {
        await upsertNormalizedProduct('cmrgcn9g80005h2qgsljije3j', 'cmrgctkaj01f3h2qgsuwnf13u', normalized);
        success++;
      } catch (upsertError) {
        failed++;
        errorSet.add(upsertError.message || String(upsertError));
      }
    } catch (e) {
      failed++;
