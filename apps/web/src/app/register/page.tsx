...
import { loginWithGoogleIdToken, registerUser } from "@/lib/auth-client";
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    script.defer = true;
    script.onload = () => {
      const mountPoint = document.getElementById("google-signup-btn");
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async (response) => {
          ...
          if (!credential) {
            setMessage("Google signup failed. Missing credential.");
            return;
          }
  ...
  return (
    <main className="mx-auto w-full max-w-lg space-y-8 px-4 py-10">
      ...
      <form className="space-y-4 border border-zinc-300 p-6" onSubmit={onSubmit}>
        <input name="fullName" type="text" placeholder="Full name" className="h-11 w-full border border-zinc-300 px-3" required />
