...
import { ApiRequestError, getBrandPendingFixProductById } from "@/lib/api";
import type { Product } from "@/types/marketplace";

export default function BrandPendingProductDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      ...
      try {
        const result = await getBrandPendingFixProductById(productId);
        if (!active) return;
        setProduct(result);
      } catch (err) {
        if (!active) return;
  ...
  return (
    <main className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10 lg:px-10">
      <header className="space-y-2 border-b border-zinc-300 pb-4">
        <p className="text-xs uppercase tracking-[0.14em] text-zinc-500">Brand Ingestion</p>
