...
type PageProps = { params: Promise<{ id: string; groupId: string; exchangeRequestId: string }> };
...
export default async function ExchangeTrackerPage({ params }: PageProps) {
  ...
  const { id, groupId, exchangeRequestId } = await params;
  const requests = await fetchJson<ReturnRequestRecord[]>(`/orders/me/${id}/sub-orders/${groupId}/return-requests`, token);
  const request = requests.find((entry) => entry.id === exchangeRequestId) || requests.find((entry) => entry.preferredResolution?.startsWith("EXCHANGE")) || requests[0];
