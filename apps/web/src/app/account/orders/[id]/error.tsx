...
export default function AccountOrderDetailError({ reset }: { reset: () => void }) {
  return (
    <main className="mx-auto w-full max-w-5xl space-y-4 px-4 py-10 lg:px-10">
      <section className="space-y-3 border border-amber-300 bg-amber-50 p-5">
        ...
        <p className="text-sm text-amber-900">We could not load this order right now.</p>
        <div className="flex gap-2">
          ...
          <Link href="/account/orders" className="inline-flex h-10 items-center border border-zinc-300 px-4 text-xs font-semibold uppercase tracking-[0.12em]">
            Back to orders
          </Link>
        </div>
      </section>
