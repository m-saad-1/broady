...
const breakoutPath = './Data_Scrapping/breakout/breakout_fashion_marketplace.json';
...
async function test(file) {
  ...
  for (const item of data) {
    ...
    if (classification.confidence < 0.7 || isFallback || isMisc || !classification.category) {
      if (issues < 20) {
        console.log(- [] Title: "" | RawCat: "/" | ProdType: "" -> Class:  / );
      }
      issues++;
    }
  }
  console.log(Total potential issues in : );
}
